...
它和 `supportman` 分开维护：`supportman` 负责当前工作处理、双源检索和发现知识反馈；`feishu-knowledge-capture` 只在你确认后，把 SupportMan 的“第二大脑反馈建议”或其他值得保留的材料写成个人 case 文档或待审核候选。
...
```text
确认，把 SupportMan 的第二大脑反馈建议整理成待审核候选。
```
...
- 未经你确认，不会因为 SupportMan 推荐沉淀就自动创建候选。
